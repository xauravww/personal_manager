...
async function main() {
    ...
    const saltRounds = 12; // Same as in auth.ts
    ...
    await prisma.learningProgress.create({
        data: {
            ...
            chat_history: JSON.stringify([
                { role: 'assistant', content: 'Welcome! Let\'s learn about variables.', timestamp: new Date().toISOString() },
                { role: 'user', content: 'What is the difference between let and const?', timestamp: new Date().toISOString() },
                { role: 'assistant', content: 'Great question! `let` allows reassignment while `const` creates a constant reference.', timestamp: new Date().toISOString() }
            ]),
    ...
    console.log('\n' + '='.repeat(50));
    console.log('📧 Email: test@learning.com');
    console.log('🔑 Password: test123');
    console.log('='.repeat(50));
    console.log(`\n🎯 Module ID: ${module1.id}`);
    console.log(`\n✨ You can now login and view the completed module with personalized data!`);
}
