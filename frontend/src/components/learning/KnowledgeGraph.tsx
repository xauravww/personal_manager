...
const getLayoutedElements = (nodes: Node[], edges: Edge[], direction = 'LR') => {
    ...
    dagreGraph.setGraph({
        rankdir: direction,
        nodesep: 50,      // Horizontal spacing between nodes (reduced from default 50)
        ranksep: 80,      // Vertical spacing between ranks (reduced from default 50)  
...
// Helper function to get node styling based on progress
const getNodeStyle = (node: Node) => {
    const { type, completion, lastActivity } = node.data;
    ...
    // Color based on completion for modules and subjects
    if ((type === 'module' || type === 'subject') && typeof completion === 'number') {
        if (completion >= 90) {
            backgroundColor = '#10B981'; // green - completed
            borderColor = '#059669';
        } else if (completion >= 50) {
            backgroundColor = '#F59E0B'; // yellow - in progress
            borderColor = '#D97706';
        } else if (completion > 0) {
            backgroundColor = '#EF4444'; // red - weak
            borderColor = '#DC2626';
        }
    } else if (type === 'resource') {
        backgroundColor = '#8B5CF6'; // purple for resources
        borderColor = '#7C3AED';
    }

    // Border thickness based on recent activity
    const daysSinceActivity = lastActivity
        ? (Date.now() - new Date(lastActivity).getTime()) / (1000 * 60 * 60 * 24)
        : 999;
    const borderWidth = daysSinceActivity < 7 ? 3 : 1;
...
const KnowledgeGraph = ({ onNodeClick, onNavigate }: { onNodeClick?: (node: Node) => void, onNavigate?: (node: Node) => void }) => {
    ...
    const fetchGraph = useCallback(async () => {
        ...
        try {
            ...
            // Apply progress-based styling to nodes
    ...
    const getStyledEdges = useCallback(() => {
        return edges.map(edge => {
            ...
            // Visual differentiation for auto-generated connections
            const isAutoGenerated = edge.data?.auto_generated === true;
            const strokeColor = isAutoGenerated ? '#8B5CF6' : '#64748b'; // purple for auto, gray for manual
            const strokeWidth = edge.data?.strength ? edge.data.strength * 3 : 1.5;

            return {
                ...edge,
                animated: edge.data?.strength > 0.8 || isAutoGenerated,
                style: {
                    stroke: isHighlighted ? '#60a5fa' : strokeColor,
                    strokeWidth: isHighlighted ? strokeWidth + 1 : strokeWidth,
                    opacity: isDimmed ? 0.2 : 1,
                    strokeDasharray: isAutoGenerated ? '5,5' : 'none', // Dashed for auto-generated
                    transition: 'all 0.3s ease'
                },
                label: edge.data?.reason || '',
                labelStyle: {
                    fontSize: 10,
                    fill: '#94a3b8',
                    fontWeight: 500
                ...
                <Panel position="top-left" className="bg-void-900/95 p-4 rounded-xl border border-starlight-100/20 backdrop-blur-md shadow-xl">
                    ...
                    <div className="space-y-3 text-xs">
                        <div className="space-y-2">
                            <p className="text-starlight-400 font-medium text-xs mb-2">Node Types</p>
                        ...
                        <div className="border-t border-starlight-100/10 pt-3 space-y-2">
                            <p className="text-starlight-400 font-medium text-xs mb-2">Progress Status</p>
                            <div className="flex items-center gap-3">
                                <div className="w-3 h-3 rounded-full bg-green-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]"></div>
                                <span className="text-starlight-200 font-medium">Completed (≥90%)</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-3 h-3 rounded-full bg-yellow-500 shadow-[0_0_8px_rgba(245,158,11,0.6)]"></div>
                                <span className="text-starlight-200 font-medium">In Progress (50-89%)</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-3 h-3 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]"></div>
                                <span className="text-starlight-200 font-medium">Weak (&lt; 50%)</span>
                            </div>
                        </div>
                        <div className="border-t border-starlight-100/10 pt-3">
                            <p className="text-starlight-300 text-[10px] leading-relaxed">
                                <span className="font-semibold">Thicker borders</span> = studied in last 7 days
                            </p>
                        </div>
                    </div>
                </Panel>
            ...
            {selectedNode && (
                <div className="absolute top-4 right-4 w-80 bg-void-900/95 backdrop-blur-md border border-starlight-100/20 rounded-xl shadow-2xl p-6 z-50 animate-in slide-in-from-right-10">
                    ...
                    <div className="space-y-4">
                        ...
                        {/* Progress Stats for Modules and Subjects */}
                        {(selectedNode.type === 'module' || selectedNode.type === 'subject') && typeof selectedNode.data.completion === 'number' && (
                            <div className="border-t border-starlight-100/10 pt-4">
                                <span className="text-xs font-medium text-starlight-500 uppercase tracking-wider block mb-3">Progress Stats</span>

                                {/* Completion Bar */}
                                <div className="mb-4">
                                    <div className="flex justify-between text-xs mb-1">
                                        <span className="text-starlight-400">Completion</span>
                                        <span className="text-starlight-200 font-semibold">{selectedNode.data.completion}%</span>
                                    </div>
                                    <div className="h-2 bg-void-800 rounded-full overflow-hidden">
                                        <div
                                            className={`h-full transition-all duration-300 ${selectedNode.data.completion >= 90 ? 'bg-green-500' :
                                                selectedNode.data.completion >= 50 ? 'bg-yellow-500' : 'bg-red-500'
                                                }`}
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {selectedNode.data.score !== null && selectedNode.data.score !== undefined && (
                                        <div className="bg-void-800/50 rounded-lg p-2">
                                            <p className="text-[10px] text-starlight-500 uppercase tracking-wider">Score</p>
                                            <p className="text-lg font-bold text-starlight-100">{Math.round(selectedNode.data.score)}%</p>
                                        </div>
                                    )}
                                    {selectedNode.data.timeSpent > 0 && (
                                        <div className="bg-void-800/50 rounded-lg p-2">
                                            <p className="text-[10px] text-starlight-500 uppercase tracking-wider">Time Spent</p>
                                            <p className="text-lg font-bold text-starlight-100">{selectedNode.data.timeSpent}m</p>
                                        </div>
                                    )}
                                    {selectedNode.data.lastActivity && (
                                        <div className="bg-void-800/50 rounded-lg p-2 col-span-2">
                                            <p className="text-[10px] text-starlight-500 uppercase tracking-wider">Last Activity</p>
                                            <p className="text-sm text-starlight-200">
                                                {new Date(selectedNode.data.lastActivity).toLocaleDateString('en-US', {
                                                    month: 'short',
                                                    day: 'numeric',
                                                    year: 'numeric'
                                                })}
